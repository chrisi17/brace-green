...
class Executor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        if not agent:
            agent = Agent(self.writeups_path)
            self.agents[context_id] = agent
            print(f"📝 Created new agent instance for context: {context_id[:16]}... (Total contexts: {len(self.agents)})")
        else:
            print(f"♻️  Reusing existing agent for context: {context_id[:16]}... (Total contexts: {len(self.agents)})")
